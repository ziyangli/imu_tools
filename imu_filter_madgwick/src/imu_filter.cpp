...
ImuFilter::ImuFilter(ros::NodeHandle nh, ros::NodeHandle nh_private):
    ...
    // **** get paramters
    nh_private_.param("use_mag", use_mag_, true);
    nh_private_.param("publish_tf", publish_tf_, true);
    nh_private_.param("reverse_tf", reverse_tf_, false);
    nh_private_.param("fixed_frame", fixed_frame_, std::string("odom"));
    nh_private_.param("constant_dt", constant_dt_, 0.0);
    nh_private_.param("publish_debug_topics", publish_debug_topics_, false);
    ...
    // **** register publishers
    imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(ros::names::resolve("imu") + "/data", 5);
...
void ImuFilter::imuCallback(const ImuMsg::ConstPtr& imu_msg_raw) {
    ...
    madgwickAHRSupdateIMU(ang_vel.x, ang_vel.y, ang_vel.z,
                          lin_acc.x, lin_acc.y, lin_acc.z,
                          dt);

    // imu_msg_raw is changed by other pointer!!!
...
void ImuFilter::imuMagCallback(const ImuMsg::ConstPtr& imu_msg_raw,
    ...
    if (!initialized_) {
        ...
        if(!std::isfinite(mag_fld.x) ||
           !std::isfinite(mag_fld.y) ||
           !std::isfinite(mag_fld.z))
            return;

        computeRPY(lin_acc.x, lin_acc.y, lin_acc.z,
                   mx, my, mz,
                   roll, pitch, yaw);
    ...
    madgwickAHRSupdate(ang_vel.x, ang_vel.y, ang_vel.z,
                       lin_acc.x, lin_acc.y, lin_acc.z,
                       mx, my, mz,
                       dt);
...
void ImuFilter::publishRawMsg(const ros::Time& t, float roll, float pitch, float yaw) {
    geometry_msgs::Vector3Stamped rpy;
    rpy.vector.x = roll;
    rpy.vector.y = pitch;
    rpy.vector.z = yaw ;
...
void ImuFilter::computeRPY(float ax, float ay, float az,
...
void ImuFilter::madgwickAHRSupdate(float gx, float gy, float gz,
...
void ImuFilter::madgwickAHRSupdateIMU(float gx, float gy, float gz,
                                      float ax, float ay, float az,
                                      float dt) {
    float recipNorm;
    float s0, s1, s2, s3;
